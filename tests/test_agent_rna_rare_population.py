...
def test_rare_study_group_enlarges_then_retains_full_reference_markers(
    ...
    def assess(**kwargs: Any) -> Any:
        ...
        action = rna_tuning.TuningAction(
            ...
            evidenceIds=[
                f"candidate:{chosen['candidateId']}",
                *evidence["imageHashes"],
            ],
            quantitativeFindings=[
                "The full reference retains the 24-cell condition group and the candidate has marker and stability evidence."
            ],
            qualitativeFindings=[
                "The observed marker/PCA board supports the distinct NKG7/GNLY cytotoxic program."
            ],
            objectivePreservation="Retain the rare condition group; do not merge or discard its cells because screening underrepresents it.",
            rationale="Accept the supported full-cohort partition after both samples lacked rare-group evidence.",
        )
    ...
    run = rna_tuning.RnaTuningRun(
        ...
        SimpleNamespace(
            config=AutomatedWorkflowConfig(screeningCells=100, maxScreeningCells=200)
        ),
        plan,
        handoff,
        study,
    ...
    assert 0 < rare_rows[0]["screeningCells"] < rare_rows[1]["screeningCells"] < 20
    assert rare_rows[2]["screeningCells"] == 24
    assert coverage[0]["coverageConcerns"] and coverage[1]["coverageConcerns"]
    assert run.evaluations["sample0"] == run.evaluations["sample1"] == []
    assert summary["budget"]["scopes"]["sample0"]["reserved"]["partitions"] == 0
    assert summary["budget"]["scopes"]["sample1"]["reserved"]["partitions"] == 0
