...
def make_bed_from_gff(
    gff: str, up_offset: int = 2000, valid_ids: List[str] = None, flavour: str = "body"
):
...
def meld_assay(
    ...
    peaks_bed = _create_bed_from_coord_ids(
        assay.feats.fetch_all(peaks_col), invalid_ids=ignore_ids
    )
    cross_idx_map = _convert_ids_to_idx(
        pd.Series(assay.feats.fetch_all(peaks_col)),
